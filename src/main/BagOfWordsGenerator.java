...
public class BagOfWordsGenerator {
	...
	public static void init() throws IOException {
		HashMap<Bigram, Integer> wordMapping2 = new HashMap<Bigram, Integer>();
...
//						wordMapping2.put(Bigram.getBigram(bigram), Integer.parseInt(value));
//					}
//
//				}
	...
	public static void generateBagOfWordsFile(boolean containment)
		...
		for (String file : documentsAdjusted) {
			...
			while (scanner2.hasNext()) {
				...
				if(mapping.containsKey(String.valueOf(wordMapping.get(bigram)))) {
					int val = wordMapping.get(bigram);
					val++;
					mapping.put(wordMapping.get(bigram), val);
				} else {
					if(wordMapping.containsKey(bigram)) {
						mapping.put(wordMapping.get(bigram), 1);
					}
				}
				if(wordMapping.containsKey(bigram)) {
					sortedVector.add(wordMapping.get(bigram));
				}
			}
			...
			for(int i : sortedList) {
				int index = i +1;
				vectorLine += index +":" + mapping.get(i) + " ";
			}
			vectorLine += "\n";
			count++;
		}
